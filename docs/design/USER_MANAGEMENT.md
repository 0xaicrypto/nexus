...
~/Library/Application Support/Nexus/nexus_server.db
  └─ users 表(权威源)
...
**核心原则**：identity.json 是 picker 索引，`users` 表（在 `nexus_server.db` 里）才是权威源。两者不一致时**永远以 DB 为准**，identity.json 可以**完全从 DB 重建**。这意味着删了 identity.json 或者文件 corrupt 都**不丢任何数据**。
