...
| Linux    | `pnpm tauri build` (on Linux) | `src-tauri/target/release/bundle/appimage/*.AppImage`, `*.deb` |

`scripts/build-macos.sh` is the only fully automated script today; it
bootstraps Homebrew, Python 3.12, pnpm, Rust, installs the three local
Python packages (`nexus-core`, `nexus`, `nexus-server`), PyInstaller-
bundles the backend, and finally runs `pnpm tauri:build`.
